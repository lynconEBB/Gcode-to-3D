...
def ambient_occlusion(script):
    filter_xml = ''.join([
        '   <filter name="Ambient Occlusion">\n',
        '        <Param type="RichEnum" value="1" enum_val1="per-Face (deprecated)" enum_cardinality="2" name="occMode" enum_val0="per-Vertex" description="Occlusion mode:"/>\n',
        '        <Param type="RichFloat" value="0" name="dirBias" description="Directional Bias [0..1]"/>\n',
        '        <Param type="RichInt" value="128" name="reqViews" description="Requested views"/>\n',
        '        <Param type="RichPoint3f" z="0" x="0" name="coneDir" y="1" description="Lighting Direction"/>\n',
        '        <Param type="RichFloat" value="30" name="coneAngle" description="Cone amplitude"/>\n',
        '        <Param type="RichBool" value="true" name="useGPU" description="Use GPU acceleration"/>\n',
        '        <Param type="RichInt" value="512" name="depthTexSize" description="Depth texture size(should be 2^n)"/>\n',
        '   </filter>\n'])
    util.write_filter(script, filter_xml)
    return None

def faces_longer_than(script,value=0.9):
    filter_xml = ''.join([
