...
def per_triangle(script):

    filter_xml = ''.join([
        '  <filter name="Parametrization: Trivial Per-Triangle">\n',
        '    <Param value="0" type="RichInt" name="sidedim" description="Quads per line"/>\n',
        '    <Param value="1024" type="RichInt" name="textdim" description="Texture Dimension (px)"/>\n',
        '    <Param value="2" type="RichInt" name="border" description="Inter-Triangle border (px)"/>\n',
        '    <Param enum_cardinality="2" value="1" type="RichEnum" enum_val0="Basic" name="method" enum_val1="Space-optimizing" description="Method"/>\n',
        '  </filter>\n'])
    util.write_filter(script, filter_xml)
    return None
